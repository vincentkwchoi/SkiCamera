...
class MainViewModel: ObservableObject {
    ...
    private func setupInternal() async {
        ...
        guard let (cameraSession, photoOutput, device) = await setupCameraSession(position: cameraPosition) else {
            return
        }
    ...
    private nonisolated func setupCameraSession(position: CameraPosition) async -> (AVCaptureSession, AVCapturePhotoOutput, AVCaptureDevice)? {
        do {
            ...
            return (session, photoOutput, device)
        } catch {
