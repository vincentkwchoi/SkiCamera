...
## 11. AutoZoom Integration

This demo now includes an **AutoZoom** feature that automatically detecting a skier (or person) and zooming in to frame them.

The implementation follows the 3-part architecture, integrating into the shared Camera code used by the Extension.

### Architecture

- **Logic**: 
    - `Camera/Logic/SkierAnalyzer.swift`: Uses Vision framework to detect human rectangles.
    - `Camera/Logic/AutoZoomManager.swift`: Implements PID control and smoothing to calculate the optimal zoom factor.
- **ViewModel**:
    - `CamPreviewViewModel` runs the analysis loop on every video frame.
    - `MainViewModel` provides access to the `AVCaptureDevice`.

### IMPORTANT: Setup for Compilation

Because the logic files are new, they must be manually added to the **LockedExtension** target to function in the locked camera mode.

1. In Xcode Project Navigator, finding `LockedCameraCaptureExtensionDemo > Camera > Logic`.
2. Select `AutoZoomManager.swift` and `SkierAnalyzer.swift`.
3. In the **File Inspector** (Right Panel), ensure **Target Membership** is checked for:
    - `LockedCameraCaptureExtensionDemo` (Main App)
    - `LockedExtension` (Content Extension)

If these are not checked, the extension will fail to build or run.
